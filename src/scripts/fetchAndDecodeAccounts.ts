...
const HELIUS_API_KEY: string | undefined = process.env.HELIUS_API_KEY;
if (!HELIUS_API_KEY) {
...
const dummyWallet = {
    ...
    signTransaction: async (transaction: any) => transaction, // Dummy signTransaction
    signAllTransactions: async (transactions: any[]) => transactions // Dummy signAllTransactions
};
...
const fetchAndDecodeAccounts = async (): Promise<void> => {
  try {
